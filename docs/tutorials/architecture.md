...
- [otio.media_linkers]: Plugin system for writing studio or workflow specific media linkers that run after adapters read files
...
Adapters can be added to the system (outside the distribution) via JSON files that can be placed on the `OTIO_PLUGIN_MANIFEST_PATH` environment variable to be made available to OTIO.

Most common usage only cares about:
```python
# Read from file
timeline = otio.adapters.read_from_file(input_file_path)

# Read to String
timeline = otio.adapters.read_from_string(raw_text, adapter_name)

# Write to File
otio.adapters.write_to_file(timeline, output_file_path)

# Write to String
raw_text = otio.adapters.write_to_string(timeline, adapter_name)
```
...
Media linkers run on the otio file after an adapter calls `.read_from_file()` or `.read_from_string()`.  They are intended to replace media references that exist after the adapter runs (which depending on the adapter are likely to be [otio.schema.MissingReference]) with ones that point to valid files in the local system.  Since media linkers are plugins, they can use proprietary knowledge or context and do not need to be part of OTIO itself.
...
Example scripts are located in the [examples subdirectory](https://github.com/PixarAnimationStudios/OpenTimelineIO/tree/main/examples).


[Write Media Linker]: write-a-media-linker.md
[Write Adapter]: write-an-adapter.md
[otio.adapters]: ../api/python/opentimelineio.adapters.html
[otio.media_linkers]: ../api/opentimelineio.media_linker
[otio.opentime]: ../api/python/opentimelineio.opentime.html
[otio.schema.Clip]: ../api/python/opentimelineio.schema.html#opentimelineio.schema.Clip
[otio.schema.ExternalReference]: ../api/python/opentimelineio.schema.html#opentimelineio.schema.ExternalReference
[otio.schema.Gap]: ../api/python/opentimelineio.schema.html#opentimelineio.schema.Gap
[otio.schema.MissingReference]: ../api/python/opentimelineio.schema.html#opentimelineio.schema.MissingReference
[otio.schema.Stack]: ../api/python/opentimelineio.schema.html#opentimelineio.schema.Stack
[otio.schema.Timeline]: ../api/python/opentimelineio.schema.html#opentimelineio.schema.Timeline
[otio.schema.Track]: ../api/python/opentimelineio.schema.html#opentimelineio.schema.Track
[otio.schema.Transition]: ../api/python/opentimelineio.schema.html#opentimelineio.schema.Transition
[otio.schema]: ../api/python/opentimelineio.schema.html
[Adapter.read_from_file]: ../api/python/opentimelineio.adapters.html#opentimelineio.adapters.Adapter.read_from_file
[otio.adapters.read_from_file]: ../api/python/opentimelineio.adapters.html#opentimelineio.adapters.read_from_file
[otio.adapters.write_to_file]: ../api/python/opentimelineio.adapters.html#opentimelineio.adapters.write_to_file